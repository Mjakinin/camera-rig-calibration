...
MOVING_DET = Path("results/bus_real_data/01_marker_direct_relay_multimarker_multichain/.ap01_compat_cache/moving_observations/moving_detections.csv")
COLMAP_IMAGES = Path("results/bus_real_data/01_marker_direct_relay_multimarker_multichain/04_moving_camera_colmap_trajectory/sparse_txt_best/images.txt")

OUT = Path("results/bus_real_data/01_marker_direct_relay_multimarker_multichain/04_moving_camera_colmap_trajectory/aruco_metric_scale")
...
def main():
    ...
    # Keep the legacy JSON scale file synchronized with the canonical scale text file.
    # Downstream AP01 relay scripts read this JSON file for the COLMAP metric scale.
    scale_json = OUT.parent / "metric_colmap_scale.json"
    scale_json.write_text(json.dumps({
        "metric_colmap_scale": float(scale),
        "source": "12_estimate_colmap_scale_from_aruco.py",
        "note": "No-GT metric scale estimated from ArUco marker side length.",
        "scale": float(scale),
        "metric_scale": float(scale),
        "scale_source": str(scale_file.relative_to(OUT.parent)),
    }, indent=2) + "\n")
