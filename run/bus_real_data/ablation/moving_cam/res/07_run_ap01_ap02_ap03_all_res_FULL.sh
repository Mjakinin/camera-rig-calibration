...
ABL_ROOT="results/bus_real_data/ablation/moving_cam/res"
...
install_variant_as_shared_baseline() {
  ...
  cp -a "$variant_raw" "$BASE_RAW"
  cp -a "$variant_obs" "$BASE_OBS"
...
run_ap01() {
  ...
  bash "$AP01_PIPELINE" 2>&1 | tee "$log"
...
run_ap02() {
  ...
  bash "$AP02_PIPELINE" 2>&1 | tee "$log"
...
run_ap03() {
  ...
  bash "$AP03_PIPELINE" 2>&1 | tee "$log"
...
main() {
  ...
  echo
  echo "[OK] all AP01/AP02/AP03 resolution ablation runs finished."
  echo "[INFO] results:"
