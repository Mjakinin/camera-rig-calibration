...
# Gazebo Sim resource paths for model://... includes.
MODEL_RESOURCE_PATHS="$PWD/src/calib_lab/bus_real_data/models"
export GZ_SIM_RESOURCE_PATH="$MODEL_RESOURCE_PATHS:$PWD/src/calib_lab/bus_real_data/worlds:${GZ_SIM_RESOURCE_PATH:-}"
export IGN_GAZEBO_RESOURCE_PATH="$GZ_SIM_RESOURCE_PATH"
export GAZEBO_MODEL_PATH="$MODEL_RESOURCE_PATHS:${GAZEBO_MODEL_PATH:-}"

if command -v gz >/dev/null 2>&1; then
  GZ_SIM_CMD=(gz sim)
  GZ_SERVICE_CMD=(gz service)
elif command -v ign >/dev/null 2>&1; then
  GZ_SIM_CMD=(ign gazebo)
  GZ_SERVICE_CMD=(ign service)
else
  echo "[ERROR] neither 'gz' nor 'ign' command found."
  exit 1
fi
...
if [ -z "$VARIANT" ]; then
  echo "Usage:"
  echo "  ./run/bus_real_data/ablation/moving_cam/fov/02_capture_one_res_variant.sh res_640x360"
  echo
  echo "Variants:"
  echo "  res_640x360"
  echo "  res_960x540"
...
WORLD_ROOT="src/calib_lab/bus_real_data/worlds/ablation/moving_cam/fov"
OUT_ROOT="results/bus_real_data/ablation/moving_cam/fov/00_captures"
...
wait_for_world_name() {
  ...
  while true; do
    local services
    services="$("${GZ_SERVICE_CMD[@]}" -l 2>/dev/null || true)"
...
echo
echo "============================================================"
echo "[RUN] moving_cam_fov single FOV variant: $VARIANT"
echo "============================================================"
echo "[INFO] world:"
echo "  $WORLD_FILE"
echo "[INFO] output:"
...
"${GZ_SIM_CMD[@]}" -r -s "$WORLD_FILE" > "$GZ_LOG" 2>&1 &
GZ_PID=$!
