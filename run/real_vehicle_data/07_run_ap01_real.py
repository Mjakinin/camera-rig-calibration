...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--top-moving-per-marker", type=int, default=8)
    parser.add_argument("--scale-top-per-marker", type=int, default=30)
...
def robust_scale(
    ...
    for marker, rows in by_marker.items():
        rows = sorted(rows, key=lambda r: r["_quality"], reverse=True)[:top_per_marker]
...
def top_moving_by_marker(rows: list[dict], registered_frames: set[int], maximum: int) -> dict[int, list[dict]]:
...
def main() -> None:
    ...
    try:
        ...
        moving_info_path = dataset / "raw_images" / "camera_info" / "moving_calib_camera.json"
        ...
        moving_by_marker = top_moving_by_marker(
            moving_rows,
            set(colmap_poses),
            args.top_moving_per_marker,
        )
        ...
        write_status(out, {
            ...
            "available_static_cameras": sorted(poses),
            "runtime_seconds": time.time() - started,
            "pose_file": str(final_dir / "AP01_STATIC_CAMERA_POSES_CAM3_REFERENCE.csv"),
        ...
        print("pose file:", final_dir / "AP01_STATIC_CAMERA_POSES_CAM3_REFERENCE.csv")

        if len(poses) < 4:
            raise RuntimeError(f"AP01 produced only {len(poses)}/4 static camera poses")
