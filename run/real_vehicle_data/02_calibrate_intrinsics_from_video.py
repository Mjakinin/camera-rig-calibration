    ...
    while frame_index < reported_frames:
        ...
        if frame_index >= next_progress_frame:
            print(
                "[SCAN]",
                f"pass={pass_label}",
                f"candidate={tested}/{len(candidate_indices)}",
                f"source_frame={frame_index}/{reported_frames}",
...
def main() -> None:
    ...
    if image_paths and args.scan_mode == "balanced":
        ...
        print(
            "[INFO] compatibility scan: every original frame at full resolution",
            flush=True,
        )
        ...
        for frame_index, image_path in enumerate(image_paths):
            ...
            if frame_index % 10 == 0 or frame_index + 1 == reported_frames:
                print(
                    "[SCAN]",
                    "pass=image-folder",
                    f"candidate={frame_index + 1}/{reported_frames}",
        ...
        while True:
            ...
            if scanned_frames % 50 == 0:
                print(
                    "[SCAN]",
                    "pass=exhaustive",
                    f"candidate={scanned_frames + 1}/{reported_frames}",
                    f"source_frame={scanned_frames}/{reported_frames}",
                    f"detections={len(detections)}",
                    flush=True,
                )
