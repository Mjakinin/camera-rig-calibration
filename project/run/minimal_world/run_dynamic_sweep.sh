...
export PYTHONPATH="$PROJECT_DIR/src/calib_lab:$PROJECT_DIR/src/calib_lab/common/scripts:$PROJECT_DIR/src/calib_lab/minimal_world/scripts/checkerboard:$PROJECT_DIR/src/calib_lab/minimal_world/scripts/aruco:$PROJECT_DIR/src/calib_lab/minimal_world/scripts/charuco:$PROJECT_DIR/src/calib_lab/minimal_world/scripts/tools:${PYTHONPATH:-}"
set -u
...
export IGN_GAZEBO_RESOURCE_PATH="$PROJECT_DIR/src/calib_lab/minimal_world/models:${IGN_GAZEBO_RESOURCE_PATH:-}"

WORLD_FILE="src/calib_lab/minimal_world/worlds/dynamic/${METHOD}_${RES_NAME}.sdf"
WORLD_NAME="dynamic_${METHOD}_${RES_NAME}"
POSE_CSV="src/calib_lab/minimal_world/worlds/dynamic/scenario_poses.csv"

case "$METHOD" in
  checkerboard)
    TARGET_NAME="target_9x6_square0_12"
    EVALUATOR="src/calib_lab/minimal_world/scripts/checkerboard/checkerboard_rig_evaluator.py"
    ;;
  aruco)
    TARGET_NAME="target_aruco_6x4_marker0_15_sep0_06"
    EVALUATOR="src/calib_lab/minimal_world/scripts/aruco/aruco_rig_evaluator.py"
    ;;
  charuco)
    TARGET_NAME="target_charuco_current"
    EVALUATOR="src/calib_lab/minimal_world/scripts/charuco/charuco_rig_evaluator.py"
    ;;
  *)
...
RESULT_BASE="results/minimal_world/${METHOD}/${TARGET_NAME}/${RES_NAME}"
ANALYZER="src/calib_lab/minimal_world/scripts/tools/analyze_checkerboard_results.py"
SET_POSE="src/calib_lab/minimal_world/scripts/tools/set_gazebo_model_pose.py"
...
if [ ! -f "$WORLD_FILE" ]; then
  ...
  echo "  $PYTHON_BIN src/calib_lab/minimal_world/scripts/tools/generate_dynamic_worlds.py --method checkerboard --resolution $RES_NAME --target_uri model://checkerboard_target"
  echo "[INFO] Example for aruco:"
  echo "  $PYTHON_BIN src/calib_lab/minimal_world/scripts/tools/generate_dynamic_worlds.py --method aruco --resolution $RES_NAME --target_uri model://aruco_target"
  exit 1
fi
