...
def load_colmap_scale():
            ...
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if isinstance(v, (dict, list)):
                        ...
                        kl = k.lower()
                        if kl in preferred_keys or ("scale" in kl and "ratio" not in kl):
                            val = float(v)
                            if 0.01 < val < 100.0:
                                return val
    ...
    # Current known good run fallback. This should normally not be needed.
    print("[WARN] Could not auto-find scale JSON. Falling back to current known good scale 0.762142493734")
    return 0.762142493734
