...
class ImageGrabber(Node):
    def __init__(self, topic):
        super().__init__("moving_camera_static_boards_capture")
...
def main():
    ...
    route_out = dataset_dir / "route_commanded.csv"

    with route_csv.open() as f:
        rows = list(csv.DictReader(f))

    print(f"[INFO] Loaded {len(rows)} route frames.")
    print(f"[INFO] Dataset: {dataset_dir}")
    print(f"[INFO] Images:  {images_dir}")
    print("[INFO] Static-board mode: no board entity is moved during capture.")
    ...
    with route_out.open("w", newline="") as f:
    ...
    print(f"[OK] Commanded route written to: {route_out}")
