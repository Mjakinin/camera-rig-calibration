...
from ..method_sdk.results import (
...
def _publish_success(
    ...
    *,
    ...
    method_metadata = resolved_method_metadata(method_id)
    method_source = source / method_metadata.artifact_directory
    _materialize_semantic_tree(
    ...
    canonical_source = method_source / "canonical_method_result.json"
    canonical_csv_source = method_source / "camera_poses_6dof.csv"
    canonical_result = None
    if canonical_source.is_file():
        canonical_result = load_canonical_result(canonical_source)
        if canonical_result.method_id != method_id:
            raise RuntimeError(
                "Canonical method result belongs to "
                f"'{canonical_result.method_id}', expected '{method_id}'"
            )
        if canonical_csv_source.is_file():
            shutil.copy2(
                canonical_source, incoming / "canonical_method_result.json"
            )
            shutil.copy2(
                canonical_csv_source, incoming / "camera_poses_6dof.csv"
            )
        else:
            write_canonical_result(incoming, canonical_result)
        if not has_extrinsics and canonical_result.camera_poses:
            write_native_camera_extrinsics(extrinsics, canonical_result)
            has_extrinsics = True
    ...
    if canonical_result is not None:
        reference = canonical_result.reference_frame
        convention = canonical_result.transform_convention
    ...
    if canonical_result is not None:
        cameras = [
            pose.camera_id for pose in canonical_result.camera_poses
        ]
    ...
    result_payload = {
        ...
        "canonical_result": "canonical_method_result.json",
        "canonical_poses": "camera_poses_6dof.csv",
        "canonical_result_available": (
            canonical_result is not None
            and canonical_result.status == "available"
        ),
        "canonical_result_status": (
            canonical_result.status if canonical_result is not None else "native_only"
        ),
        "canonical_pose_count": (
            len(canonical_result.camera_poses)
            if canonical_result is not None
            else 0
        ),
    ...
    lines = [
        ...
        (
            "Canonical 6DOF: camera_poses_6dof.csv and "
            "canonical_method_result.json"
            if canonical_result is not None
            else "Canonical 6DOF: native result without SDK artifact"
        ),
        "Diagnostics: diagnostics/",
        "Complete logs: logs/",
