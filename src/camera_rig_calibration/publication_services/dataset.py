...
def _method_status(source: Path, method_id: str) -> dict[str, Any]:
    method_root = (
        source / resolved_method_metadata(method_id).artifact_directory
    )
    candidates = [
...
def _reference_metadata(method_id: str, status: dict[str, Any]) -> tuple[str, str]:
    if method_id == "ap01":
        ...
        reference = str(
            status.get("reference_frame") or "method-defined reference frame"
        )
...
def _export_extrinsics(
    ...
    metadata = resolved_method_metadata(method_id)
    pose_source = (
        source / metadata.artifact_directory / metadata.primary_pose_path
        if metadata.primary_pose_path is not None
        else source / Path("__missing__")
    )
