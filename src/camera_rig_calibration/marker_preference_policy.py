    ...
    def setting_rows(job, groups=None):
        ...
        for key, group, label, current, baseline, description in rows:
            if key == "evaluation_anchor":
                ...
                description = (
                    f"Default prefers marker {preferred} and falls back automatically if it is unavailable or incompatible. Auto requests pure deterministic ranking; Manual lets you choose a detected marker after preflight. The legacy Baseline option remains for old strict simulation configs."
                )
