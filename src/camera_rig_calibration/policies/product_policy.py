...
def _install_reporting_policy() -> None:
    ...
    def baseline_contract(
        *,
        ...
        # AP02 uses 80/80 evaluations. Preserve every other contract check.
...
def _install_wizard_policy() -> None:
    ...
    wizard._PUBLIC_POLICY_NAMES["smart_v1"] = (
    ...
    def setting_rows(job, groups=None):
        ...
        for key, group, label, current, baseline, description in rows:
            ...
            if key == "ap02_frame_strategy":
                ...
                baseline = "cam_edge_3"
                description = (
                    "Final simulation AP01 baseline uses cam_edge_3 with the "
                    "validated Direct/Relay geometry. Change only for an "
