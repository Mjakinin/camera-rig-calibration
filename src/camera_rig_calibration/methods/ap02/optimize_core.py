"""AP02 bundle-adjustment scientific core."""
...
def load_initial_poses(mode, initialization_root=INIT_ROOT):
    mode_dir = initialization_root / mode
...
def run_ba(
    ...
    *,
    ...
    marker_init, observer_init = load_initial_poses(
        mode, initialization_root
    )
    all_rows = read_csv(observations_csv)
    static_observer_ids = {
        row["observer_id"]
        for row in all_rows
...
def main():
    ...
    initialization_root = (
        args.initialization_root.resolve()
        if args.initialization_root is not None
        else ap02_root / "05_graph_initialization"
    )
    run_ba(
        args.mode,
        args.ref_marker_id,
