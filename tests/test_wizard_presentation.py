...
def test_public_setting_rows_hide_only_ap02_frame_strategy_compatibility_id() -> None:
    source = [
        _row("ap02_frame_strategy", "smart_v1", "smart_v1"),
        _row("other_setting", "smart_v1", "smart_v1"),
    ]
    ...
    assert rendered[1][3] == "smart_v1"
    assert rendered[1][4] == "smart_v1"

    # The presentation layer must never mutate scientific source values.
    assert source[0][3] == "smart_v1"
    assert source[0][4] == "smart_v1"
